# bazi_engine v1.0.0-rc0 (BaZodiac / BAFE)
...
## Contract-first /validate (BaZodiac)

This release adds a contract-first validator endpoint:

- POST /validate
- Request/Response are bound to JSON Schema Draft-07 under ./spec/schemas/
- Canonical ruleset: ./spec/rulesets/standard_bazi_2026.json
- Determinism: use now_utc_override (no time-dependent defaults in tests)
- No implicit network calls or downloads (RefData must be explicit)
...
Response is a ValidateResponse with:
- compliance_status (COMPLIANT/DEGRADED/NON_COMPLIANT)
- component statuses (REFDATA/TIME/FRAMES/EPHEMERIS/DISCRETIZATION/REPRODUCIBILITY/INTERPRETATION_POLICY)
- errors + warnings with contract-bound error codes
- evidence object for reproducibility/audit
